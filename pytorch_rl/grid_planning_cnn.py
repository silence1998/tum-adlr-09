...
class GridWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = 10 if terminated else -0.02  # Binary sparse rewards
...
policy_net = DQN(5,5, n_actions).to(device)
target_net = DQN(5,5, n_actions).to(device)
...
for i_episode in range(num_episodes):
    ...
    env.reset()
    obs = env._get_obs()
    state = torch.zeros((5, 5), device=device)
    state[obs["agent"][0],obs["agent"][1]] = 1
    state[obs["target"][0],obs["target"][1]] = 10
    state=state.view(1,1,5,5)
    for t in count():
        ...
        obs = env._get_obs()
        if not done:
            next_state = torch.zeros((5, 5), device=device)
            next_state[obs["agent"]] = 1
            next_state[obs["target"]] = 10
            next_state = next_state.view(1,1, 5, 5)
        else:
            next_state = None
